...
function generateProxyHandler(paths: string[][], isFirst: boolean): ProxyHandler<object> {
  return {
    get(_target, property, receiver) {
      if (typeof property === "symbol") {
        // Changing the type of the error is breaking change
        // oxlint-disable-next-line unicorn/prefer-type-error
        throw new Error(`ts-nameof-proxy: The path cannot contain ${property.toString()}.`);
      }
      if (isFirst) {
        paths.push([property]);
